                ...
                match ime_event {
                    ...
                    ImeEvent::Commit(commit_text) => {
                        if commit_text == "\n" || commit_text == "\r" {
                            ...
                            // Some Linux X11/XIM integrations emit a commit without a usable
                            // preedit event. In that case `ime_cursor_range` is stale, but the
                            // non-empty commit still belongs at the current text cursor.
                            // Backport of the commit behavior from egui PR #7983.
