    ...
    R: ReadAt,
{
    ...
    let s_checksum_seed = LittleEndian::read_u32(&entire_superblock[0x270..0x274]);
    ...
    let verify_checksums = has_checksums && crate::Checksums::Ignored != options.checksums;
    ...
    let uuid_checksum = if verify_checksums {
        Some(
            if incompatible_features.contains(IncompatibleFeature::CSUM_SEED) {
                s_checksum_seed
            } else {
                ext4_style_crc32c_le(!0, &s_uuid)
            },
        )
    } else {
        None
    ...
    F: FnOnce(u64) -> Result<Vec<u8>, Error>,
{
    ...
    if load_xattrs
        && inode_end + 4 <= data.len()
        && XATTR_MAGIC == read_le32(&data[inode_end..(inode_end + 4)])
    {
        let table_start = &data[inode_end + 4..];
        read_xattrs(&mut xattrs, table_start, table_start)?;
    }

    if load_xattrs && (0 != i_file_acl_lo || 0 != l_i_file_acl_high) {
