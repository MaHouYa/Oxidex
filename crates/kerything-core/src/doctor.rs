...
pub const SCANNER_POLKIT_ACTION: &str = "net.reikooters.kerything.connect-scanner";
pub const DEFAULT_SCANNER_SOCKET: &str = "/run/kerything/scannerd.sock";
    ...
    if let Some(gid) = group_gid("kerything") {
        if meta.gid() == gid {
            report.add(
                ...
                "Scanner socket group is kerything",
            );
        } else {
            report.add(
                DoctorCategory::Scanner,
                ...
                "scanner_socket_group",
                format!(
                    "Scanner socket gid is {}, expected kerything ({gid})",
                    meta.gid()
                ),
            );
        }
        ...
        report.add(
            ...
            DoctorSeverity::Warn,
            "scanner_group",
            "System group kerything does not exist",
        );
    }
}
    ...
    if let Some(gid) = group_gid("kerything") {
        ...
        if groups.contains(&gid) {
            report.add(
                ...
                "Current process is in the kerything group",
            );
        } else {
            report.add(
                DoctorCategory::Security,
                DoctorSeverity::Warn,
                "scanner_group_membership",
                "Current user is not in the kerything group; scanner socket connection may fail before Polkit",
            );
        }
    }
...
fn check_systemd_units(report: &mut DoctorReport) {
    for (category, path) in [
        (
            "user_unit",
            Path::new("/usr/lib/systemd/user/kerythingd.socket"),
        ),
        (
            "system_unit",
            Path::new("/usr/lib/systemd/system/kerything-scannerd.socket"),
        ),
    ] {
        report.add(
