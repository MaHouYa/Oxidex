...
use oxidex_core::model::FsType;
use oxidex_core::scanner::{ScanCancellation, validate_device_path};
...
fn run() -> anyhow::Result<()> {
    ...
    if args.len() == 2 && args[1] == "--version" {
        println!("oxidex-scanner-helper v{}", oxidex_core::VERSION);
        return Ok(());
    }
    ...
    let db = oxidex_core::scanner::scan_device(
        &device,
        fs_type,
        &mut |done, total| {
    ...
    oxidex_core::stream::write_scan_stream(&mut lock, &db)?;
    lock.flush()?;
    Ok(())
}
