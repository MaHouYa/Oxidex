...
use anyhow::Context;
use kerything_client::{KerythingClient, default_daemon_socket_path};
use kerything_core::daemon_model::SearchQueryParams;
use kerything_core::doctor::{
...
fn command_search(client: &mut KerythingClient, mut args: Vec<String>) -> anyhow::Result<()> {
...
fn command_rofi(client: &mut KerythingClient, mut args: Vec<String>) -> anyhow::Result<()> {
...
fn command_scan(client: &mut KerythingClient, args: Vec<String>) -> anyhow::Result<()> {
    ...
    if wait {
        loop {
            ...
            match job.state {
                kerything_core::daemon_model::ScanState::Finished => break,
                kerything_core::daemon_model::ScanState::Failed
                | kerything_core::daemon_model::ScanState::Cancelled => {
...
fn command_copy_path(client: &mut KerythingClient, args: Vec<String>) -> anyhow::Result<()> {
    let (device_id, record_idx) = parse_hit_arg(&args)?;
    let resolved = client.resolve_path(&device_id, record_idx)?;
...
fn command_resolve(client: &mut KerythingClient, mut args: Vec<String>) -> anyhow::Result<()> {
...
fn command_config(client: &mut KerythingClient, args: Vec<String>) -> anyhow::Result<()> {
...
fn run_doctor(args: Vec<String>) -> anyhow::Result<()> {
    ...
    let mut report = run_local_doctor(&DoctorOptions {
        ...
        scanner_socket: None,
        helper_path: Some(sibling_binary("kerything-scanner-helper")),
        scanner_only,
        security_only,
    });
...
fn print_usage() {
    eprintln!(
        "Usage:\n  kerything-cli search [--json] [--limit N] [--sort relevance|name|path|size|mtime] <query>\n  kerything-cli explain <query>\n  kerything-cli rofi [--limit N] [--show-id] [--full-path] <query>\n  kerything-cli indexes\n  kerything-cli devices\n  kerything-cli scan <device-id> [--wait]\n  kerything-cli jobs\n  kerything-cli cancel <job-id>\n  kerything-cli open --hit <device-id>:<record-idx>\n  kerything-cli open-folder --hit <device-id>:<record-idx>\n  kerything-cli copy-path --hit <device-id>:<record-idx>\n  kerything-cli resolve --hit <device-id>:<record-idx> [--json]\n  kerything-cli config get\n  kerything-cli config set <path> <value>\n  kerything-cli doctor [--json] [--scanner] [--security]"
    );
}
