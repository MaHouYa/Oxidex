...
Rename note: version 2.3.0 continues the public Oxidex line. Commands, crates, config paths, index paths, systemd units, and the scanner socket group use the `oxidex` name.
...
The package is written to `dist/oxidex_2.3.0_amd64.deb`. The GitHub Actions workflow in `.github/workflows/deb.yml` builds the same package inside an `ubuntu:20.04` job container and uploads it as a workflow artifact.
